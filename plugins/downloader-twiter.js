...
let handler = async (m, { conn, text, usedPrefix, command }) => {
	if (!text)
		throw `Input URL\n\n${
			usedPrefix + command
		} https://twitter.com/dwanchy/status/1655594954633125888?s=19`;
	let res = await download(text);
	m.reply(wait);
	if (res.link.length < 1)
	...
	if (res.link.length > 1 && m.isGroup) {
		conn.sendFile(
			...
			res.link[0],
			"...",
			`Bot Akan Mengirim Sisa *${
				res.link.length - 1
			} Foto/Video* Di Private Chat`,
			m
		);
...
async function download(url) {
	try {
		...
		const Result = [];
		$(".col-md-4.col-md-offset-4 > a.btn.btn-primary.btn-sm.btn-block").each(
			(i, e) => {
