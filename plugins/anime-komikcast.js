...
let handler = async (m, { conn, usedPrefix, command, text }) => {
	if (!text && !/komikcast(latest|ongoing)|komikcast-latest/i.test(command))
		return m.reply(
			`Masukan Text!\n\nContoh:\n${
				usedPrefix + command
			} Kage No Jitsuryokusha\n${
				usedPrefix + command
			} https://komikcast.site/komik/kage-no-jitsuryokusha-ni-naritakute/`
		);
