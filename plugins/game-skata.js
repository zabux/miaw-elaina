...
let handler = async (
	...
	let room_all = Object.values(conn.skata).find(
		(room) => room.id !== id && room.player.includes(m.sender)
	);
	if (room_all)
		throw `Kamu sedang bermain sambung kata di chat lain, selesaikan game kamu terlebih dahulu`;
	if (id in conn.skata) {
			...
			if (!room.waktu._destroyed && !room.diam)
				return conn
					.reply(
						...
						`Hii @${
							m.sender.split`@`[0]
						}, Masih ada game berlangsung di chat ini\nTunggu hingga game berakhir\nLalu ikut bergabung`,
						room.chat,
		...
		if (text == "start" && room.status == "wait") {
			...
			room.waktu_list = setTimeout(() => {
				conn
					.reply(m.chat, `Sambung kata tidak dimulai (Cancel)`, room.chat)
					.then(() => {
						delete conn.skata[id];
					});
			}, 120000);
...
function filter(text) {
	...
	// voc2x + mati(optional) {portofolio, manusia, tiup, dll}
		...
		if (/(ng)$/i.test(text))
			return text.substring(text.length - 3); // ex tiang, riang, siang
