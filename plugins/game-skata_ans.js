...
handler.before = async function (m) {
		...
		room.waktu = setTimeout(() => {
			...
			this.reply(
				...
				if (member.length == 1 && room.status == "play") {
					this.reply(
						m.chat,
						`@${member[0].split`@`[0]} Berhasil bertahan\n+${room.win_point}XP`,
						room.chat,
						{ contextInfo: { mentionedJid: member } }
					).then((_) => {
	...
	if (room.curr == m.sender) {
		if (/nyerah/i.test(m.text)) {
			...
			this.reply(
				m.chat,
				`@${room.curr.split`@`[0]} tereliminasi -${lose_skata} MMR${
					room.killer ? `\n@${room.killer.split`@`[0]} +${win_skata} MMR` : ""
				}`,
				room.chat,
				{ contextInfo: { mentionedJid: member } }
