    ...
    try {
        ...
        try {
            ...
            if (settings) {
                ...
                if (!('lastcleartmp' in settings)) settings.lastcleartmp = 0
        ...
        const isSegel = db.data.users[m.sender].ilegal > 0
        ...
        if (opts['nyimak'] && !isOwner)
            return
        if (!m.fromMe && opts['self'])
            return
        if (opts['pconly'] && m.chat.endsWith('g.us'))
            return
        if (db.data.settings[this.user.jid].gconly && !m.chat.endsWith('g.us') && !isPrems && new Date() * 1 - db.data.users[m.sender].pc < 21600000)
            return
        if (db.data.settings[this.user.jid].gconly && !m.chat.endsWith('g.us') && !isPrems)
            return conn.adReply(m.chat, 'Bot Sedang Dalam Mode *Group Only* \nSilahkan Gunakan Bot Di *Group*, Atau Jika Ingin Menggunakan Bot Di *Private Chat*, Silahkan Upgrade User Ke *Premium*', wm, null, 'https://telegra.ph/file/153420f13c5a02a4c58c6.jpg', link.gc, m)
            .then(() => {
                db.data.users[m.sender].pc = new Date() * 1
            })
