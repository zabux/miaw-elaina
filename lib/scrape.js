				...
				try {
					hasil = {
						...
						message: {
							tanggal: fetchText
								.split("Tanggal: ")[1]
								.split("Karakteristik: ")[0],
							karakteristik: fetchText
...
async function tafsir_mimpi(value) {
	return new Promise((resolve, reject) => {
		axios
			.get(
				"https://primbon.com/tafsir_mimpi.php?mimpi=" +
					value +
					"&submit=+Submit+"
			)
			.then(({ data }) => {
				...
				try {
					hasil = {
						...
						message: {
							mimpi: value,
							arti: fetchText
								.split(`Hasil pencarian untuk kata kunci: ${value}`)[1]
								.split("\n")[0],
...
async function nhentai(url) {
	try {
		...
		const language = $(
			"#tags > div:nth-child(5) > span > a > span.name"
		).text();
		const Categories = $(
			"#tags > div:nth-child(6) > span > a > span.name"
		).text();
